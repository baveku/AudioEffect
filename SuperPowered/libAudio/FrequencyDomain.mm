...
@implementation FrequencyDomain {
...
static bool audioProcessing(void *clientdata, float **buffers, unsigned int inputChannels, unsigned int outputChannels, unsigned int numberOfSamples, unsigned int samplerate, uint64_t hostTime) {
    __unsafe_unretained FrequencyDomain *self = (__bridge FrequencyDomain *)clientdata;
...
-(void) setupRecord {
    ...
    _audioFilePath = [@"Audio-" stringByAppendingString:timeString];
    NSString *temp = [[NSTemporaryDirectory() stringByAppendingString:@"audio"] stringByAppendingString:timeString];
    recorder = new SuperpoweredRecorder([temp fileSystemRepresentation], 44100);
    isPauseRecord = true;
}

-(void) startRecordAudio {
    isPauseRecord = false;
    recorder->start([[NSTemporaryDirectory() stringByAppendingString:_audioFilePath] fileSystemRepresentation]);
    NSLog(@"Start record");
}
...
-(void)setupFilters:(FilterType)type withFrequency:(float)frequency Octave:(float)octave Decibel:(float)decibel Resonance:(float)resonace Slope:(float)slope dbGain:(float)dbGain enable: (bool) flag {
...
-(void) setupEchos: (float) mix enable: (bool) flag {
...
-(void) setupReverbs: (float) mix : (float) damp : (float) roomSize enable: (bool) flag {
...
-(void)setBandEQWithLowEQ: (float)low midEQ:(float)midEQ highEQ:(float)highEQ enable: (bool)flag {
    if (!bandEQ) bandEQ = new Superpowered3BandEQ(44100);
    bandEQ->reset();
    bandEQ->bands[0] = low;
    bandEQ->bands[1] = midEQ;
    bandEQ->bands[2] = highEQ;
    bandEQ->enable(flag);
}

-(void) setupFlanger : (float)wet lfoBeats: (float)lfo depth: (float)depth enable: (bool)flag{
